...
class NormalDataset(Dataset):
    def __init__(self, root_path, sample_num, keypoint_type='full'):
    ...
    def _get_item(self, index):
        ...
        samples = self.sample(p_sdf_grad, self.sample_num)

        data_dict = {
            'XYZ': samples[:, :3], 'SDF': samples[:, 3], 'GRADIENTS': samples[:, 4:7],
            'EXP_INDEX': 0, 'ID_INDEX': 0
        }
